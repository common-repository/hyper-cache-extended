	...
	if (is_admin()){
		...
		if(isset($hyper_cache)){
		    if (!is_dir($hyper_cache['path'])){
		    	...
		    	if (!is_dir($hyper_cache['path'])){
					$hyper_notice .= '<h3>Hyper Cache Extended was not able to create the folder "cache"</h3> The WordPress cache system cannot create ('.$hyper_cache['path'].'). <br/>' .
						'Create it by hand and make it writable or change the directory path in wp-content/advanced-cache.php<br />';
		    	}
		    }

		    if (!is_file(WP_CONTENT_DIR . '/advanced-cache.php')){
				$hyper_notice .= '<h3>Your wp-content/ folder is not writable.</h3> Hyper Cache Extended needs to create a file called advanced-cache.php in to that folder in order to work. ' .
					'Make it writable and deactivate and reactivate Hyper Cache.<br />';
		    }
		}
